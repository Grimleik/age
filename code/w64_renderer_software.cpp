...
void W64RendSoftware::DrawBufferClear(v4f color) {
...
void W64RendSoftware::DrawBufferLine(s32 x0, s32 y0, s32 x1, s32 y1, v4f color0, v4f color1) {
...
void W64RendSoftware::DrawBufferLine(s32 x0, s32 y0, s32 x1, s32 y1, v4f color) {
    // Clamp into buffer: "clipping ?"
    x0 = Contain(x0, 0, backBuffer.width - 1);
    y0 = Contain(y0, 0, backBuffer.height - 1);
    x1 = Contain(x1, 0, backBuffer.width - 1);
...
void W64RendSoftware::DrawBufferFillTriangle(v3f t0, v3f t1, v3f t2, v4f c0, v4f c1, v4f c2) {
...
#if USE_BARY
// barycentric fill.
void W64RendSoftware::DrawBufferFillTriangle(v3f t0, v3f t1, v3f t2, v4f c0) {
    v3f min = v3f{(f32)backBuffer.width - 1, (f32)backBuffer.height - 1, 0.0f};
    v3f max = v3f{0.0f, 0.0f, 0.0f};
    if (min.x > t0.x) min.x = t0.x;
    if (min.x > t1.x) min.x = t1.x;
